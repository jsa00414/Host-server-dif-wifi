...
NAS_FILES_SNIPPET = (
    ...
    "overflow:hidden!important;touch-action:manipulation;-webkit-text-size-adjust:100%;text-size-adjust:100%;zoom:1!important}"
    "body{position:relative!important;background:#fff}"
    ".x-viewport,.x-border-layout-ct{min-height:100%!important}"
    # Sencha Touch fullscreen panels must pin to the iframe, not a stale window size.
    ...
    "}"
    "}catch(e){}"
    # Sencha Touch (/nas-files/st/) — fullscreen Stage + bottom toolbar.
    "try{"
    ...
    "}"
    "var nodesST=document.querySelectorAll('.x-fullscreen,#maindataview');"
    "for(var si=0;si<nodesST.length;si++){"
    "nodesST[si].style.position='absolute';"
    "nodesST[si].style.left='0';nodesST[si].style.top='0';"
    "nodesST[si].style.right='0';nodesST[si].style.bottom='0';"
    "nodesST[si].style.width=w+'px';nodesST[si].style.height=h+'px';"
    "}"
    "}catch(e){}"
    "}"
    ...
    "try{window.smNasFit=smNasFit;}catch(e){}"
    "try{"
    "window.addEventListener('resize',smNasFit,false);"
    "window.addEventListener('orientationchange',function(){setTimeout(smNasFit,50);},false);"
    "if(window.visualViewport)visualViewport.addEventListener('resize',smNasFit,false);"
    "var _smFitN=0;"
    "var _smFitT=setInterval(function(){smPatchExtDom();smNasFit();if(++_smFitN>=80)clearInterval(_smFitT);},200);"
    "if(document.readyState==='complete')setTimeout(smNasFit,0);"
    "else window.addEventListener('load',function(){setTimeout(smNasFit,0);setTimeout(smNasFit,400);setTimeout(smNasFit,1200);},false);"
    "var _smExtWait=setInterval(function(){"
    "if(smPatchExtDom()&&window.Ext&&Ext.onReady){"
    "clearInterval(_smExtWait);"
    "try{Ext.onReady(function(){setTimeout(smNasFit,0);setTimeout(smNasFit,200);setTimeout(smNasFit,800);});}catch(e){}"
    "}"
    "},100);"
    "}catch(e){}"
    "function fix(u){if(typeof u!=='string')return u;"
