...
NAS_FILES_SNIPPET = (
    ...
    "h=Math.max(h,Math.floor(visualViewport.height)||0);}"
    "if(!(w>0&&h>0))return;"
    "var narrow=w<=900;"
    "if(window.Ext){"
    "try{"
    ...
    "if(fp){"
    "if(narrow){if(fp.getHeight&&fp.getHeight()>0){fp.setHeight(0);fp.hide&&fp.hide();}}"
    "else if(fp.getHeight&&fp.getHeight()===0){fp.setHeight(typeof FOOTER_HEIGHT==='number'?FOOTER_HEIGHT:15);fp.show&&fp.show();}}"
    "}catch(e){}"
    ...
    "var isBorder=(c.layout.type==='border')||(c.layout==='border')||"
    "(c.getXType&&c.getXType()==='viewport')||(c.xtype==='viewport');"
    "if(!isBorder&&!(c.el&&c.el.hasClass&&c.el.hasClass('x-viewport')))return;"
    "if(c.setSize)c.setSize(w,h);"
    "if(c.doLayout)c.doLayout(true,true);"
    "}catch(e){}"
    "});}"
    "}catch(e){}"
    "}"
    ...
    "var _smFitN=0;"
    "var _smFitT=setInterval(function(){smNasFit();if(++_smFitN>=48)clearInterval(_smFitT);},250);"
    "if(document.readyState==='complete')setTimeout(smNasFit,0);"
    "else window.addEventListener('load',function(){setTimeout(smNasFit,0);setTimeout(smNasFit,300);},false);"
    "}catch(e){}"
    "function fix(u){if(typeof u!=='string')return u;"
